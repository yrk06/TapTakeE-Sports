...
public class PlayerController {
    ...
    public ResponseEntity<?> findById(@RequestParam String id){
        return ResponseEntity.status(HttpStatus.OK).body(playerService.findById(UUID.fromString(id)));
    }
    ...
    public ResponseEntity<?> save(@RequestBody PlayerDTO playerDTO) {
        ...
        if(playerOptional.isPresent() && playerOptional.get().getCargo()==playerDTO.getCargo() && playerOptional.get().getIdEquipe()== playerDTO.getIdEquipe()){
